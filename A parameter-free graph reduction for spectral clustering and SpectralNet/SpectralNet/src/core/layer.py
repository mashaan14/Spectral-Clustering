...
def orthonorm_op(x, epsilon=1e-7):
    ...
    x_2 += K.eye(K.int_shape(x)[1])*1e-3 #epsilon 1e-3
    # x_2_symm = (x_2 + tf.transpose(x_2)) / 2
    # x_2_symm = 0.5 * (x_2 + tf.transpose(x_2))
    # x_2_upper = tf.matrix_band_part(x_2, 0, -1)
    # x_2_symm = x_2_upper + tf.transpose(x_2_upper)
    # x_2_symm = x_2 * tf.transpose(x_2)
    # x_3 = tf.cast(x_2, tf.float64)
    # L = tf.cholesky(x_2_symm)
    # x_2_transpose = tf.transpose(x_2)
    # x_2_symm = tf.matmul(x_2, x_2_transpose)
    # x_2_symm = tf.cast(x_2_symm, tf.float64)
    
    L = tf.cholesky(x_2)
    
    # L = tf.cast(L, tf.float32)
    
    # L = x * tf.constant(np.nan)
    # try:
    #     L = tf.cholesky(x_2)
    ...
    #     while jitter < 10:
    #         try:
    #             L = tf.cholesky(x_2 + tf.Diagonal(jitter))
    #         except Exception:
    #             jitter = jitter * 10
