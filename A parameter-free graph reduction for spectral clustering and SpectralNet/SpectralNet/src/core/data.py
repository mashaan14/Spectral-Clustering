...
def load_data(params):
    ...
    if params['dset'] == 'reuters':
        ...
        y_test = np.ndarray(shape=(0,), dtype=y_train.dtype)        
    elif params['dset'] == 'CircleSinCos1304':
        x_train = np.genfromtxt(os.path.join(ProjectDir,'CircleSinCos1304_Instances.csv'), delimiter=",")
        x_test = np.ndarray(shape=(0,x_train.shape[1]), dtype=x_train.dtype)
        y_train = np.genfromtxt(os.path.join(ProjectDir,'CircleSinCos1304_labels.csv'), delimiter=",")
        y_test = np.ndarray(shape=(0,), dtype=y_train.dtype)                
    else:
