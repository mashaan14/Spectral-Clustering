...
for epoch=0:param.epochs
    ...
    if FlagQE
        if epoch==0 && isequal(net.IW{1,1}(:,1),net.IW{1,1}(:,2))
    %         net.IW{1,1} = zeros(size(net.IW{1,1}));

            XLimit = linspace(min(data.X{1,1}(1,:)),max(data.X{1,1}(1,:)),round(sqrt(size(net.IW{1,1},1))));
            YLimit = linspace(min(data.X{1,1}(2,:)),max(data.X{1,1}(2,:)),round(sqrt(size(net.IW{1,1},1))));
            [GridX,GridY]=meshgrid(XLimit,YLimit); GridZ=zeros(size(GridX));
            A=[GridX(:) GridY(:) GridZ(:)];
            OrthonormalBasis = pca(data.X{1,1}','NumComponents',2);
            ProjMat =  OrthonormalBasis * ((OrthonormalBasis' * OrthonormalBasis) \ OrthonormalBasis');
            B = ProjMat * A';
            net.IW{1,1} = B';
        end
        try
    ...
    NeuronsDistances = evalin('base', 'NeuronsDistances');
    if FlagPlot
        figure(1);
        PlotResults(data.X{1,1}', net.IW{1,1}, NeuronsDistances);
        pause(0.01);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [minDist,~] = pdist2(net.IW{1,1},transpose(data.X{1,1}),'squaredeuclidean','Smallest',1);
